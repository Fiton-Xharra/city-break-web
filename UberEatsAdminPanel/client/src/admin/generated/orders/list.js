...
import IMOrdersProductForeignKeysArrayTableCell from '../tableCells/IMOrdersProductForeignKeysArrayTableCell.js'
...
const ordersColumns = [
        ...
        // },
    {
        ...
        accessor: "products",
        Cell: data => (
            <IMOrdersProductForeignKeysArrayTableCell productsArray={data.value} />
        )
    },
    {
        Header: "Customer",
        accessor: "authorID",
        Cell: data => (
            <IMForeignKeyTableCell id={data.value} apiRouteName="user" titleKey="firstName" />
        )
    },
    {
        Header: "Restaurant",
        accessor: "vendorID",
        Cell: data => (
            <IMForeignKeyTableCell id={data.value} apiRouteName="restaurant" titleKey="title" />
        )
    },
    {
        Header: "Status",
...
function ActionsItemView(props) {
    ...
    const handleView = (item) => {
        const viewPath = "/admin/order/" + item.id + '/view'
        history.push(viewPath)
    }

    const handleEdit = (item) => {
        const editPath = "/admin/order/" + item.id + '/update'
        history.push(editPath)
    }

    const handleDelete = async (item) => {
        if (window.confirm('Are you sure you want to delete this item?')) {
            const response = await fetch(baseAPIURL + 'order/' + item.id, {
...
function OrdersListView(props) {
    ...
    useEffect(() => {
        ...
        const extraQueryParams = "?orderBy=createdAt&desc=true"
        setIsLoading(true)
    ...
    useEffect(() => {
        ...
        setOrders(data.slice(startRow, endRow))
                    ...
                    <Col md="12">
                        <div className="Card">
                            <div className="CardHeader">
                                <a className="Link AddLink" href="/admin/order/add">Add New</a>
                                <h4>Orders</h4>
                            </div>
                                ...
                                <div className="TableContainer">
                                    <table className="Table" {...getTableProps()}>
                                        ...
                                        <tbody {...getTableBodyProps()}>
                                            ...
                                            <tr>
                                                {isLoading ? (
                                                    <td colSpan={ordersColumns.length - 1}>
                                                        <p>
                                                            Loading...
                                                        </p>
                                                    </td>
